...
class IntroScreen:
    def __init__(self, master, duration=3000):
        """
        A modern, theme-adaptive splash screen for SMS Detector.
        """
        self.master = master
        ...
        self._center_window()  # initial placement
        self.alpha = 0.0
        self.root.attributes("-alpha", self.alpha)

        # Keep above, then allow normal stacking
        self.root.lift()
        self.root.attributes("-topmost", True)
        self.root.after_idle(self.root.attributes, "-topmost", False)

        # Hide the main window during splash
        self.master.withdraw()

        # Delay showing the splash a tick to avoid Win compositor white flash
        self.root.withdraw()
        ...
        # Theme-safe canvas background
        mode = ctk.get_appearance_mode().lower()
        bg_color = "#F9F9F9" if mode == "light" else "#0D1117"

        # Minimal vector shield
        self.canvas = tk.Canvas(
            title_row, width=26, height=26,
            bg=bg_color, highlightthickness=0, bd=0, relief="flat"
        )
        self.canvas.pack(side="left", padx=(0, 10))  # more breathing room
        self.base_shield_color = "#1e88e5"
        self.shield_shape = self.canvas.create_polygon(
            13, 2, 23, 8, 23, 17, 13, 24, 3, 17, 3, 8,
            fill=self.base_shield_color, outline=self.base_shield_color
        )

        # Elegant mixed-weight title
        ...
        title_bold = ctk.CTkLabel(
            title_row, text="Detector",
            font=ctk.CTkFont(family="Segoe UI", size=24, weight="bold"),
            text_color=("#1e88e5", "#58a6ff"),
        )
        title_bold.pack(side="left", anchor="s")
        ...
        # Animations (prepared; start in _show_and_start)
        self.loading_states = itertools.cycle(["Starting.", "Starting..", "Starting..."])

    # ----- internal helpers -------------------------------------------------

    def _center_window(self):
        """Reliable screen-centering for the toplevel."""
    ...
    def _show_and_start(self):
        """Show splash after Tk has painted, then start animations."""
        self._center_window()
        self.root.deiconify()
        self._fade_in()
        self._animate_loading()
        self._update_progress()
        self._animate_shield_pulse()
    ...
    def _animate_shield_pulse(self):
        """Subtle pulse that does not drift from the base color."""
        up = getattr(self, "_pulse_up", True)
        factor = 1.12 if up else 0.88

        def tint(hex_color, k):
        ...
        new_color = tint(self.base_shield_color, factor)
        self.canvas.itemconfig(self.shield_shape, fill=new_color, outline=new_color)
        self._pulse_up = not up
        self.root.after(420, self._animate_shield_pulse)
